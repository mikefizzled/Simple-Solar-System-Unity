...
public class Controller : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.LeftShift) && Input.GetKey(KeyCode.UpArrow))
            ...
            current.orthographicSize -= speed * Time.deltaTime;
        ...
        if (birdsEye.enabled)
        {
            if (Input.GetKey(KeyCode.A))
                current.transform.position -= new Vector3(speed * Time.deltaTime, 0, 0);
            if (Input.GetKey(KeyCode.D))
                current.transform.position += new Vector3(speed * Time.deltaTime, 0, 0);
            if (Input.GetKey(KeyCode.W))
                current.transform.position += new Vector3(0, 0, speed * Time.deltaTime);
